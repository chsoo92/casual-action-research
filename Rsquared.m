...
for reference = [0 1] %causal for 0 and non-causal for 1   
    ...
    for subj = 1:length(datafiles)
        ...
        if reference == 0
            rsquared_causal(subj,1) = 1 - sum((proportion'-logitFit).^2)/sum((proportion-mean(proportion)).^2); 
            causalRefPSE(subj,1) = PSEspeed;
            rsquared_causal_logitGLM(subj,1) = glm.Rsquared.ordinary;
            ADrsquared_causal_logitGLM(subj,1) = glm.Rsquared.Adjusted;
        else
            rsquared_noncausal(subj,1) = 1 - sum((proportion'-logitFit).^2)/sum((proportion-mean(proportion)).^2); 
            noncausalRefPSE(subj,1) = PSEspeed;
            rsquared_noncausal_logitGLM(subj,1) = glm.Rsquared.ordinary;
            ADrsquared_noncausal_logitGLM(subj,1) = glm.Rsquared.Adjusted;
        end
...
%the calculated Rsquared v.s. Rsquared from built-in function
